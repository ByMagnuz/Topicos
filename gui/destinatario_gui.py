...
class DestinatarioGUI(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        self.formDireccion = QtWidgets.QLineEdit(self)
        self.formDireccion.setObjectName("formDireccion")
        self.formIdMunicipio = QtWidgets.QLineEdit(self)
        self.formIdMunicipio.setObjectName("formIdMunicipio")
        self.formLayout.addRow("Nombre:", self.formNombre)
        self.formLayout.addRow("Dirección:", self.formDireccion)
        self.formLayout.addRow("ID Municipio:", self.formIdMunicipio)
    ...
    def edit(self):
        ...
        if row != -1:
            ...
            id_destinatario = int(self.tblDatos.item(row, 0).text())
            # Obtener los datos del registro para editar
            data = self.bd.obtenerDatosPorID(id_destinatario)
            if data:
                # Mostrar los datos en el formulario de edición
                self.formNombre.setText(data.get('nombre', ''))
                self.formDireccion.setText(data.get('direccion', ''))
                self.formIdMunicipio.setText(str(data.get('id_municipio', '')))
                self.editing_id = id_destinatario  # Establecer el ID de edición
    ...
    def save(self):
        ...
        direccion = self.formDireccion.text()
        id_municipio = self.formIdMunicipio.text()
        if nombre.strip() and direccion.strip() and id_municipio.strip().isdigit():  # Validar que los campos no estén vacíos y que id_municipio sea un número
            id_municipio = int(id_municipio)
            if self.editing_id:
                mensaje = self.bd.actualizar(self.editing_id, nombre, direccion, id_municipio)
            ...
            self.formDireccion.clear()
            self.formIdMunicipio.clear()
