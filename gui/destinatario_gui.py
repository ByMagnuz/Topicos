...
class DestinatarioGUI(QtWidgets.QWidget):
    ...
    def insert(self):
        nombre, ok = QtWidgets.QInputDialog.getText(self, 'Nuevo Destinatario', 'Nombre:')
        if not ok or not nombre:
            return
        direccion, ok = QtWidgets.QInputDialog.getText(self, 'Nuevo Destinatario', 'Dirección:')
        if not ok or not direccion:
            return
